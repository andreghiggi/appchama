...
export default function LoginScreen() {
  const { login } = useAuth();
  const [phone, setPhone] = useState('5511999990003');
  const [name, setName] = useState('Roberto Alves');
  ...
  const verify = async () => {
    setLoading(true);
    try {
      const result = await api.verifyOtp(phone, code, name, 'driver');
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Chama Motorista</Text>
      {step === 'phone' ? (
        <>
        ...
        <>
          <TextInput style={styles.input} placeholder="OTP" value={code} onChangeText={setCode} maxLength={6} />
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: theme.ink, padding: 24, justifyContent: 'center' },
  title: { fontSize: 24, fontWeight: '700', color: '#fff', marginBottom: 24 },
  input: { backgroundColor: theme.paper, borderRadius: 12, padding: 14, marginBottom: 12 },
  button: { backgroundColor: theme.amber, borderRadius: 13, padding: 16, alignItems: 'center' },
